...
<template>
	...
	<div class="skill">
		...
		<span class="separate" @mouseenter.capture.stop="dispatch('itemover', $event, skill )">
			<span>{{ skill.name }}</span>&nbsp;
			<span v-if="skill.owned">{{ 'lvl: ' + skill.value.toFixed(2) + '/' + Math.floor(skill.max) }}<button class="train-btn"
